#!/usr/bin/env bash
#
# lint 抑制コメントの禁止: Edit / Write / apply_patch で biome-ignore / eslint-disable などの
...
# Codex の apply_patch は tool_input.command にパッチ本文が入る。
if [ -z "$file" ] && [ "$tool_name" = "apply_patch" ]; then
  patch_cmd="$(jq -r '.tool_input.command // empty' <<< "$input")"
  [ -z "$patch_cmd" ] && exit 0
  # 追加行(+...)だけを対象。ファイルパスはヘッダから拾う。
  content="$(printf '%s\n' "$patch_cmd" | grep -E '^\+' | grep -vE '^\+\+\+' || true)"
  file="$(printf '%s\n' "$patch_cmd" | grep -E '^\*\*\* (Add|Update) File: ' | head -1 | sed -E 's/^\*\*\* (Add|Update) File: //' || true)"
elif [ -n "$file" ]; then
  case "$tool_name" in
...
# 設定・依存・フック自身などは対象外。
case "$file" in
  */.claude/*|*/.codex/*|*/node_modules/*|*/dist/*|*/src-tauri/target/*) exit 0 ;;
  .claude/*|.codex/*|node_modules/*|dist/*|src-tauri/target/*) exit 0 ;;
esac
