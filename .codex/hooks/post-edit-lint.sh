#!/usr/bin/env bash
#
# 編集後の自動リントチェック: Edit / Write / apply_patch されたファイルに oxlint / Biome を実行し、
...
single_file="$(jq -r '.tool_input.file_path // .tool_input.path // empty' <<< "$input")"
if [ -n "$single_file" ]; then
  ...
  while IFS= read -r path; do
    [ -n "$path" ] && files+=("$path")
  done < <(printf '%s\n' "$patch_cmd" | grep -E '^\*\*\* (Add|Update) File: ' | sed -E 's/^\*\*\* (Add|Update) File: //')
fi
...
lint_one() {
  local file="$1"
  local diag="" ran_lint=false start_dir
...
for file in "${files[@]}"; do
  file_diag="$(lint_one "$file" || true)"
  if [ -n "$file_diag" ]; then
    all_diag="${all_diag}${file_diag}
"
  fi
done
