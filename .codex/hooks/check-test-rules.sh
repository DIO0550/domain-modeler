...
check_one() {
  ...
  local config_file="" disable_line="" violations="" matches="" basename=""
...
for file in "${files[@]}"; do
  file_violations="$(check_one "$file" || true)"
  if [ -n "$file_violations" ]; then
    all_violations="${all_violations}${file_violations}
"
  fi
done
