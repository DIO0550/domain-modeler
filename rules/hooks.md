...
- 背景タブの編集状態を保持する場合も、非表示のエディタのグローバル購読を動作させ続けない。`Activity` 等で非表示時に購読を解除し、キャンバスの pan だけでなく要素のドラッグ・リサイズを含む操作中のポインター状態も cleanup でキャンセルする。
- `Activity` からの復帰で Effect が再接続されても、状態遷移が無い自動フォーカスを再実行しない。タブバーなど、復帰操作を行った要素のフォーカスを維持する。
- 同じ外部購読を複数のEffect世代が共有できる場合、購読側を参照カウントまたは登録IDで所有権管理する。古いEffectの遅延cleanupで新しい世代の購読を停止しない。
- ファイル監視など同一対象の非同期イベントは文書単位で直列化し、古い読み込みの完了が新しい内容を上書きしないようにする。
