...
- 新規作成は保存先選択後の競合も含めて既存ファイルを上書きしない排他的な操作を使う。hard link 非対応のボリュームでも `create_new` 相当の排他性を維持する。編集中のパスへの新規作成は、文字列表現ではなくファイルシステム上の同一性で判定し、ディスクと編集セッションの不一致を避けるため拒否する。
