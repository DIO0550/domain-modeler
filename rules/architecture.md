...
- 新規作成は保存先選択後の競合も含めて既存ファイルを上書きしない排他的な操作を使う。hard link 非対応のボリュームでも `create_new` 相当の排他性を維持し、一時ファイル名は保存先名の長さに依存しない上限のある名前にする。編集中のパスへの新規作成は、`..` より先に存在する親のシンボリックリンクを解決し、対象ディレクトリ固有の大小文字規則を含むファイルシステム上の同一性で判定して、ディスクと編集セッションの不一致を避けるため拒否する。
