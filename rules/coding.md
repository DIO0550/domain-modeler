...
- アプリケーション終了要求では、開いている全ドキュメントの未保存変更を flush してから終了する。1件でも保存に失敗した場合は終了を中止し、編集内容を保持する
- 終了時の flush 中も編集可能な画面では、flush開始時のスナップショットだけで終了可否を決めない。保存操作の世代が変わっていないことを再確認し、全ドキュメントが同時に保存済みになるまで終了しない
- UI イベントループから呼ぶ可能性があるファイル I/O は blocking executor へ移し、低速・ネットワークファイルシステムでも画面を停止させない
- 外部ファイル変更を取り込む前に未保存編集との競合を判定し、ユーザーが保持する側を選ぶまで自動保存と終了を停止する
- 保存処理中に外部変更を検出した場合、進行中の書き込み完了を待ってファイルを再読込し、安定した内容で競合を再評価する。書き込み中のスナップショットを競合選択へ出さない
- 終了保存と外部変更の競合判定には、文書履歴だけでなく、フォーカス中の入力欄に残る未確定の編集下書きも含める
- 外部変更の適用結果がundo/redo履歴を持つ場合、現在値だけをUIへ渡して履歴を再生成しない。ドメインが生成した履歴全体を編集セッションへ引き継ぐ
- 外部削除イベントでは保留中・進行中の保存を停止してから実在を再確認し、未保存編集や未確定下書きがあれば、削除を維持するか編集内容で再作成するかを明示的に解決するまで書き込みを再開しない
- ファイル監視の開始後にOSの監視バックエンドが失敗した場合もエラーをUIへ伝播し、外部状態を再確認できるかユーザーが明示的に上書きを選ぶまで自動保存を再開しない
- 外部変更イベントでもファイル内容の読み込み前に保留中の自動保存を停止し、非同期I/O後の最新状態から未保存内容と未確定下書きを再取得する。外部状態を確認できなければ、再確認または明示的な上書き選択まで保存を再開しない
...
- 三項演算子をネストしない(`no-nested-ternary` で強制)。複数条件の分岐は早期リターンや名前を付けた値で表現する
...
- コメント・JSDoc は日本語で書く。公開コンポーネント・カスタムフックの JSDoc には、責務と `@param` / `@returns` を記す
